...
export default function Home() {
  ...
  const carouselSlides = [
    {
      ...
      overlay: "linear-gradient(135deg, rgba(11,26,43,0.6) 0%, rgba(212,175,55,0.4) 100%)",
      textColor: "#ffffff",
      buttonBg: "#d4af37"
    },
    {
  ...
  const topServices = [
    ...
    { name: "Life Insurance", description: "Customized life plans for long-term financial security.", image: "https://cdn-icons-png.flaticon.com/512/825/825509.png" },
    { name: "Health Insurance", description: "Comprehensive plans covering medical emergencies and wellness.", image: "https://cdn-icons-png.flaticon.com/512/2966/2966328.png" },
    { name: "Business Registration", description: "Legally register your business to start your journey with confidence.", image: "https://cdn-icons-png.flaticon.com/512/1006/1006555.png" },
  ];
  ...
  useEffect(() => {
    ...
    // step px per tick; interval ms - tuned for smoothness & speed
    let step = isMobile ? 0.8 : 1.2; // px per tick (slower on mobile)
    let tick = 16; // ms
    let rafId = null;
    let stopped = false;
    ...
    function loop() {
      ...
      if (!el) return;
      // if at end, reset to start with a small delay to avoid jump flash
      if (Math.ceil(el.scrollLeft + el.clientWidth) >= el.scrollWidth) {
        // smooth reset: animate back to 0
        el.scrollTo({ left: 0, behavior: "smooth" });
      } else {
        el.scrollLeft = el.scrollLeft + step;
      }
      rafId = window.setTimeout(loop, tick);
    }
  ...
  const servicesGridStyle = {
    ...
    overflowX: "auto",
    paddingBottom: "8px",
    // hide native scrollbar on some browsers (visual only; still scrollable)
  ...
  const serviceCardStyle = {
    ...
    minWidth: isMobile ? "240px" : "300px", // fixed min width so cards align horizontally
    flex: "0 0 auto",
  };
    ...
    <div style={{ width: '100vw', margin: 0, padding: 0, overflowX: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={heroSectionStyle}>
        <AnimatePresence mode="popLayout">
          <motion.div
            key={currentSlide}
            style={{ ...slideStyle, backgroundImage: `url(${carouselSlides[currentSlide].backgroundImage})` }}
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -50 }}
            transition={{ duration: 0.8 }}
          >
            <div style={{ ...overlayStyle, background: carouselSlides[currentSlide].overlay }}>
      ...
      <section style={sectionStyle}>
        ...
        >
          <div style={{...servicesInnerStyle, paddingBottom: 6, marginLeft: isMobile ? 8 : 0}}>
          {topServices.map((service, index) => (
            <motion.div 
              key={index} 
              style={serviceCardStyle} 
              whileHover={isMobile ? {} : { y: -8, boxShadow: "0 15px 40px rgba(0,0,0,0.1)" }}
            >
      ...
      <section style={aboutSectionStyle}>
        <div style={aboutContentStyle}>
          ...
          <div style={aboutVisualStyle}>
            <h3 style={{ marginBottom: "20px", fontWeight: "700", fontSize: isMobile ? "1.3rem" : "1.5rem" }}>
              Excellence in Numbers
            </h3>
            <div style={statsGridStyle}>
              <div style={statItemStyle}><div style={statNumberStyle}>5,000+</div><div style={statLabelStyle}>Satisfied Clients</div></div>
              <div style={statItemStyle}><div style={statNumberStyle}>12+</div><div style={statLabelStyle}>Years Expertise</div></div>
              <div style={statItemStyle}><div style={statNumberStyle}>98%</div><div style={statLabelStyle}>Success Rate</div></div>
              <div style={statItemStyle}><div style={statNumberStyle}>24/7</div><div style={statLabelStyle}>Client Support</div></div>
            </div>
          </div>
        </div>
      </section>
